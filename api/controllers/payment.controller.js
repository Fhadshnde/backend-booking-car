...
export const createPaymentIntent = catchAsync(async (req, res, next) => {
  ...
  const booking = await Booking.findById(bookingId).populate("carId brand model").populate("companyId name");
...
export const confirmPayment = catchAsync(async (req, res, next) => {
  ...
  if (paymentIntent.status === "succeeded") {
    ...
    booking.paymentMethod = "credit_card_fake";
    await booking.save();

    await sendNotification({
      userId: booking.userId,
      title: "تم الدفع بنجاح",
      message: `تأكيد دفع وهمي للحجز رقم ${booking.confirmationCode}`,
      type: "payment_received",
      relatedBooking: booking._id
    });

    res.status(200).json({ success: true, message: "تم التأكيد بنجاح", booking });
  }
});

export const stripeWebhook = async (req, res) => {
  res.json({ received: true, mode: "fake" });
};
